...
class OBOROSelectITEMbyAmountGenerated:
    ...
    def _count_files(folder: Path, debug: bool = False) -> int:
        try:
            ...
            OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated][ERROR] Counting files in '{folder}': {e}")
            return 0
    ...
    def _build_gen_stats(gen_root: Path, gen01: Path, gen02: Path, prompt_folder: Path, debug: bool = False) -> dict:
        ...
        return {
            ...
            'gen_count': int(OBOROSelectITEMbyAmountGenerated._count_files(gen_root, debug=debug)) if gen_root.exists() else 0,
            'gen01_folder': OBOROSelectITEMbyAmountGenerated._safe_str(gen01 if gen01.exists() else None),
            'gen01_count': int(OBOROSelectITEMbyAmountGenerated._count_files(gen01, debug=debug)),
            'gen02_folder': OBOROSelectITEMbyAmountGenerated._safe_str(gen02 if gen02.exists() else None),
            'gen02_count': int(OBOROSelectITEMbyAmountGenerated._count_files(gen02, debug=debug)),
    ...
    def _file_exists_and_has_text(file_path: Path, debug: bool = False) -> bool:
        try:
            ...
            OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated][ERROR] Reading prompt file '{file_path}': {e}")
            return False
    ...
    def _check_prompt_requirements(prompt_folder: Path, prompt_requirements: dict, name: str, debug: bool) -> bool:
        ...
        if not any(prompt_requirements.values()):
            if debug:
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated]     No prompt requirements enabled - accepting all items")
            return True
        
        if debug:
            required_list = [k for k, v in prompt_requirements.items() if v]
            OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated]     Required prompts: {', '.join(required_list)}")
            OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated]     Prompt folder exists: {prompt_folder.exists()}")
        ...
        for model_type, required in prompt_requirements.items():
            if required:
                ...
                file_exists = OBOROSelectITEMbyAmountGenerated._file_exists_and_has_text(prompt_file, debug=debug)
                if debug:
                    OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated]       {model_type}: {prompt_file.name} exists: {file_exists}")
                if not file_exists:
        ...
        if missing_prompts:
            if debug:
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated]     [FAIL] Missing required prompts: {', '.join(missing_prompts)}")
            return False
        
        if debug:
            OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated]     [PASS] All required prompts found")
        return True
    ...
    def _discover_items_simple(project_root: Path,
        ...
        OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated] === DISCOVERY (SIMPLE - {structure_type}) ===")
        OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated] Searching in: '{root}'")
        OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated] Project: '{project_name}'")
        ...
        if png_in_item_root:
            ...
            OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated] Found {len(png_files)} PNG files in root")
            
            if len(png_files) == 0:
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated][WARNING] No PNG files found in root directory!")
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated] Expected structure: base_dir/ITEM.png with ITEM/prompt/ and ITEM/gen/")
                # List what IS in the directory
                all_files = list(root.iterdir())
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated] Directory contains {len(all_files)} items:")
                for item in all_files[:10]:  # Show first 10
                    item_type = "DIR" if item.is_dir() else "FILE"
                    OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated]   [{item_type}] {item.name}")
                if len(all_files) > 10:
                    OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated]   ... and {len(all_files) - 10} more items")
            
            for png_file in png_files:
                name = png_file.stem
                item_dir = root / name
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated] Checking PNG: '{png_file.name}' -> item name: '{name}'")
                
                if not item_dir.is_dir():
                    OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated]   [SKIP] No corresponding folder '{name}' found")
                    continue  # Skip if no corresponding folder exists
                
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated]   [OK] Folder exists: '{item_dir}'")
                ...
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated]   Checking subfolders:")
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated]     prompt/: {prompt_folder.exists()}")
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated]     gen/: {gen_root.exists()}")
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated]     gen/01/: {gen01.exists()}")
                ...
                prompt_check = OBOROSelectITEMbyAmountGenerated._check_prompt_requirements(prompt_folder, prompt_requirements, name, debug)
                if not prompt_check:
                    OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated]   [SKIP] Failed prompt requirements check")
                    continue
                else:
                    OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated]   [OK] Passed prompt requirements check")
                ...
                items.append({
                    ...
                    **OBOROSelectITEMbyAmountGenerated._build_gen_stats(gen_root, gen01, gen02, prompt_folder, debug=debug),
                })
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated]   [ADDED] Item '{name}': gen={items[-1]['gen_count']}, gen01={items[-1]['gen01_count']}, gen02={items[-1]['gen02_count']}")
        else:
            ...
            OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated] Found {len(item_dirs)} subdirectories")
            
            if len(item_dirs) == 0:
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated][WARNING] No subdirectories found!")
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated] Expected structure: base_dir/ITEM/ITEM.png with ITEM/prompt/ and ITEM/gen/")
                # List what IS in the directory
                all_files = list(root.iterdir())
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated] Directory contains {len(all_files)} items:")
                for item in all_files[:10]:  # Show first 10
            ...
            for item_dir in item_dirs:
                ...
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated] Checking folder: '{name}'")
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated]   Expected PNG: '{item_png.name}' exists: {item_png.exists()}")
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated]   prompt/: {prompt_folder.exists()}")
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated]   gen/: {gen_root.exists()}")
        ...
        OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated] === DISCOVERY COMPLETE: {len(items)} items added ===")
        return items
    ...
    def _discover_items_nestedupscale(project_root: Path,
        ...
        if debug:
            OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated] Discovering items (nestedupscale structure) in project: '{project_name}' at {root}")
        ...
        for category in sorted([p for p in root.iterdir() if p.is_dir() and not p.name.startswith('.')], key=lambda x: x.name.lower()):
            if include_categories and category.name not in include_categories:
                continue
            if debug:
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"  [Category] {category.name}")
            for group in sorted([p for p in category.iterdir() if p.is_dir() and not p.name.startswith('.')], key=lambda x: x.name.lower()):
                ...
                if upscale.exists():
                    ...
                    for p in list(group.glob('*.psd')) + list(group.glob('*.bmp')):
                        ...
                        names.add(p.stem)
                if debug:
                    OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"    [Group] {group.name} -> candidates: {len(names)}")
                for name in sorted(names):
                    ...
                    if debug:
                        OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"      [Item] {name}: gen01={items[-1]['gen01_count']}, gen02={items[-1]['gen02_count']}, prompt={'yes' if items[-1]['prompt_folder'] else 'no'}")
        return items
    ...
    def _select_by_amount(all_items, max_images_target=5, debug: bool = False):
        ...
        if debug:
            OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated] Discovered {len(all_items)} items, created schedule with {len(schedule)} entries")
        return schedule
    ...
    def _create_balanced_schedule(items, max_images_target=5, debug: bool = False):
        ...
        if debug:
            OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated] Balanced Schedule Calculation:")
            OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"  Target image count: {max_images_target}")
            OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"  Actual max image count: {actual_max}")
            OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"  Using max count: {max_count}")
            for item, reps in schedule_entries[:5]:  # Show first 5
                total = item.get('gen_count', 0) + item.get('gen01_count', 0) + item.get('gen02_count', 0)
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"    {item.get('name')}: total_images={total}, repetitions={reps}")
        ...
        if debug:
            OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated] Created schedule with {len(schedule)} entries")
            # Show first 10 entries
            OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"[SelectITEMbyAmountGenerated] First 10 schedule entries:")
            for i, item in enumerate(schedule[:10]):
                total = item.get('gen_count', 0) + item.get('gen01_count', 0) + item.get('gen02_count', 0)
                OBOROSelectITEMbyAmountGenerated._debug_print(debug, f"    {i}: {item.get('name')} (total_images={total})")
